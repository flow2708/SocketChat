...
class ClientSomthing {
    ...
    public ClientSomthing(String addr, int port) {
        ...
        this.port = port;
        int attempts = 0;
        while (attempts < 5) { // Пытаемся подключиться 5 раз
            try {
                this.socket = new Socket(addr, port);
                break; // Если подключились, выходим из цикла
            } catch (IOException e) {
                attempts++;
                System.err.println("Connection failed, attempt " + attempts + "/5");
                try {
                    Thread.sleep(1000); // Ждём 1 секунду перед повторной попыткой
                } catch (InterruptedException ignored) {}
            }
        }
        if (socket == null) {
            System.err.println("Could not connect to server after 5 attempts. Exiting.");
            System.exit(1); // Завершаем программу, если не удалось подключиться
        }
        try {
